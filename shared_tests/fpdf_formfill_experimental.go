...
var _ = Describe("fpdf_formfill_experimental", func() {
	...
	Context("a PDF file with a form annotation with color", func() {
		var doc references.FPDF_DOCUMENT
		var formHandle references.FPDF_FORMHANDLE
		formHistory := []FormHistory{}
		timers := map[int]*FormTicker{}
		...
		BeforeEach(func() {
			formHistory = []FormHistory{}
			pdfData, err := ioutil.ReadFile(TestDataPath + "/testdata/text_form_color.pdf")
			Expect(err).To(BeNil())
		...
		When("is opened", func() {
			It("allows experimental form methods to be called", func() {
				...
				Expect(err).To(BeNil())
				Expect(FPDFAnnot_GetFontColor).To(Equal(&responses.FPDFAnnot_GetFontColor{
					R: 255,
					G: 128,
					B: 0,
				}))
